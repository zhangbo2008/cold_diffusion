...
#上采样逆回去的层
    ...
    def forward(self, x, time):
        orig_x = x
        t = self.time_mlp(time) if exists(self.time_mlp) else None #时间进行编码
...
#img为加完噪音的图.
        ...
        while(t):
            ...
            x = self.denoise_fn(img, step) # x是xt重构会原始图片, 论文中的x0hat,也就是direct_recon. 一步重构原图片.

            if self.train_routine == 'Final':
                if direct_recons == None: # 首先直接重构设置为x
                    direct_recons = x

                if self.sampling_routine == 'default': # 对应论文里面的
                    if self.blur_routine == 'Individual_Incremental':
                ...
                elif self.sampling_routine == 'x0_step_down': #/////////这个是算法里面的核心算法2.
                    x_times = x
                    for i in range(t):            
                        with torch.no_grad():
                            x_times = self.gaussian_kernels[i](x_times) # x_times是公式里面的xt
                            if self.discrete:
...
# 运行到这的时候x_times=D(x0,s)
                    x_times_sub_1 = x   #  s:t-->1: x(s-1)=xs-D(x0,s)+D(x0,s-1)
                    for i in range(t - 1):
                        with torch.no_grad():
                            x_times_sub_1 = self.gaussian_kernels[i](x_times_sub_1)
#=======运行到这时候x_times_sub_1=D(x0,s-1)  # 其实比较就能看出来437到444 跟446到449是两个相似的过程. 没啥难度就是按照论文里面的写上而已!!!!!!!!!!!!!
    ...
    def q_sample(self, x_start, t):
        ...
        # Remember to do convs without torch.grad  #用conv来实现blur过程.
        max_iters = torch.max(t) # 输入 x0和t
        all_blurs = []
        x = x_start
        for i in range(max_iters+1):
            with torch.no_grad():
                x = self.gaussian_kernels[i](x) # 每一次都是高斯过滤. 把所有的过滤都走一遍.不用求导的.每一步都记录下来.
                if self.discrete:
                    if i == (self.num_timesteps-1):
        ...
        all_blurs = torch.stack(all_blurs) # all_blurs: torch.Size([47, 32, 1, 32, 32])        x.shape:torch.Size([32, 1, 32, 32])   
...
# choose_blur 表示每一次模糊之后的结果.
    ...
    def p_losses(self, x_start, t): # 核心的损失函数!!!!!!!!! x_start是原始图像, t是时间轴乱序的.
        b, c, h, w = x_start.shape
        if self.train_routine == 'Final':
            x_blur = self.q_sample(x_start=x_start, t=t) # 对应D函数,x_blur是模糊后的图片.
            x_recon = self.denoise_fn(x_blur, t) # R函数resotre函数.也就是去噪.
    ...
    def forward(self, x, *args, **kwargs):
        ...
        t = torch.randint(0, self.num_timesteps, (b,), device=device).long() # 创建乱序的时间轴. size是32大小.
...
class Trainer(object):
    def __init__(
        ...
        *,
        ...
        if dataset == 'mnist' or dataset == 'cifar10' or dataset == 'flower' or dataset == 'celebA' or dataset == 'AFHQ':
            print(dataset, "使用了数据")
            self.ds = Dataset_Aug1(folder, image_size) # 数据提升.
    ...
    def train(self):
        ...
        backwards = partial(loss_backwards, self.fp16)  # bp函数.
...
# 1000轮开始test一下.
    ...
    def fid_distance_decrease_from_manifold(self, fid_func, start=0, end=1000, pytorch_msssim=None):
